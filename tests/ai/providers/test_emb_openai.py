...
def _fake_create_embeddings(*, model, texts, **kwargs):
    ...
    return types.EmbeddingResult(
        result=types.EmbeddingVectors(embeddings=[[float(len(text)), 1.0] for text in texts]),
...
def test_get_embeddings_single_string_returns_vector(monkeypatch):
    from velari_ai.integrations.openai.client import OpenAIProviderEmbeddings

    embedder = OpenAIProviderEmbeddings(api_key="test-key", embedding_model="text-embedding-3-small")
    monkeypatch.setattr(embedder._client, "create_embeddings", _fake_create_embeddings)
