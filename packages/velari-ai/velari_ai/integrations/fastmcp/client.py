...
class MCPClient(object):
    ...
    async def discover_tools(self, verbose: bool = False) -> List[Tool]:
        ...
        try:
            ...
            raise
        if verbose:
            df = pd.DataFrame([tool.model_dump(exclude_none=True) for tool in result.tools])
            df["display_name"] = [get_display_name(tool) for tool in result.tools]
            logger.info(f"\n{df[['display_name', 'name', 'description', 'inputSchema']].to_string()}")
        return result.tools
    ...
    async def discover_resources(self, verbose: bool = False) -> List[Resource]:
        ...
        try:
            ...
            raise
        if verbose:
            df = pd.DataFrame([resource.model_dump(exclude_none=True) for resource in result.resources])
            df["display_name"] = [get_display_name(resource) for resource in result.resources]
            logger.info(f"\n{df[['display_name', 'uri', 'name', 'description']].to_string()}")
    ...
    async def discover_prompts(self, verbose: bool = False) -> List[Prompt]:
        ...
        try:
            ...
            raise
        if verbose:
            df = pd.DataFrame([prompt.model_dump(exclude_none=True) for prompt in result.prompts])
            df["display_name"] = [get_display_name(prompt) for prompt in result.prompts]
            logger.info(f"\n{df.to_string()}")
